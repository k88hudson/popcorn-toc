...
"use strict";
	...
	Popcorn.plugin( 'supertext' , function(options, base) {
		var popcorn,
			video,
			classes,
			container,
		...
		if (!styleSheet) {
			...
			styleSheet.appendChild(
				document.createTextNode(
					'.popcorn-supertext { display: none; }\n' +
					'.popcorn-supertext.active { display: block; }\n'
			));
		...
		if (options.innerContainer) {
			textContainer = getElementByID(options.innerContainer);
		} else if (!options.innerContainer && options.link) {
			//todo: localize link
